...
local function unFreezePlayer(p, musicPaused, soundPaused)
...
function costume.onTick()
    if SaveData.toggleCostumeAbilities then
        for _,p in ipairs(Player.get()) do
            ...
            --[[
            for _, v in pairs(NPC.get(291, p.section)) do
                graphx.boxLevel(v.x, v.y, v.width, v.height, 0xFF000066)
                for _, w in pairs(NPC.get(horikawaTools.hittableNPCs, p.section)) do
            ...
            for _, npcLister in pairs(NPC.get(NPC.HITTABLE, p.section)) do
            ...
            --end
            if(not killed and p:mem(0x13E,FIELD_BOOL)) then
                killed = true
                Audio.SfxStop(-1)
            ...
            --Jumps
             if (not p:isGroundTouching() and p:mem(0x34, FIELD_WORD) ~= 2) and (p:mem(0x48, FIELD_WORD) == 0) then
                if p:mem(0x40, FIELD_WORD) ~= 3 then
                ...
                if  p.jumpKeyPressing or p.altJumpKeyPressing then
                    if p.speedY > 0.2 then 
                        p.speedY = 2.5
                    end
                    --Hover timer
                    --p:mem(0x1C, FIELD_WORD) == 25
                end            
            else
                canJump = false
                hasJumped = false
            ...
            --Quit level
            if p.keys.run == KEYS_PRESSED and isInStartMenu then
                unFreezePlayer(p, false, false)
                exitLevel()
            end
        end
...
function costume.onInputUpdate()
    if SaveData.toggleCostumeAbilities then
        for _,p in ipairs(Player.get()) do
            pm.winStateCheck()
            if p.keys.altRun == KEYS_PRESSED and costume.usesavestate then
                if not isInStartMenu then
                    oldGravity = Defines.gravity
                end
                isInStartMenu = not isInStartMenu
            elseif p.keys.jump == KEYS_PRESSED or p.keys.altJump == KEYS_PRESSED then
                --prevent hover 
                p:mem(0x1C, FIELD_WORD, 0)
                if (canJump) then
                    p.speedY = -10
                    playSFX(1)
                    NPC.spawn(291, p.x, p.y, p:mem(0x15A, FIELD_WORD))
                    canJump = false
                end
            end
        end
