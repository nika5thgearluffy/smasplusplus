...
function costume.shootLaser1(plr)
...
function costume.onControllerButtonPress(button, playerIdx)
    if SaveData.toggleCostumeAbilities then
            if button == SaveData.SMASPlusPlus.player[Player(playerIdx)].controls.specialButton then
                if (player.powerup ~= 5) then
                    if smasCharacterGlobals.abilitySettings.rebelTrooperCanShootBlaster then
                        costume.shootLaser1(Player(playerIdx))
                    end
                end
            end
        end
...
function costume.onTick()
    if SaveData.toggleCostumeAbilities then
        for _,p in ipairs(Player.get()) do
            if p:isOnGround() or p:isClimbing() then --Checks to see if the player is on the ground, is climbing, is not underwater (smasFunctions), the death timer is at least 0, the end state is none, or the mount is a clown car
                hasJumped = false
            elseif (not hasJumped) and p.keys.jump == KEYS_PRESSED and p.deathTimer == 0 and Level.endState() == 0 and p.mount == 0 and not Playur.underwater(player) then
...
function costume.onDraw()
    if SaveData.toggleCostumeAbilities then
        ...
        if smasCharacterGlobals.abilitySettings.rebelTrooperCanUseCustomHurtSystem then
            ...
            if plr.forcedState == FORCEDSTATE_POWERDOWN_SMALL or plr.forcedState == FORCEDSTATE_POWERDOWN_FIRE or plr.forcedState == FORCEDSTATE_POWERDOWN_ICE then
                plr.forcedState = FORCEDSTATE_NONE
                plr:mem(0x140, FIELD_WORD, 150)
            end
...
function costume.hphit()
    if SaveData.toggleCostumeAbilities then
        if smasCharacterGlobals.abilitySettings.rebelTrooperCanUseCustomHurtSystem then
            if not plr.hasStarman and not plr.isMega then
