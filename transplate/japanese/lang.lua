return {
    ...
    ["0x0000000000000001"] = "<setPos 400 32 0.5 -1.2><boxStyle smw>スーパーマリオコレクション++へようこそ<page>このゲームは、スーパー マリオ ブラザーズ 1 ~ 3、スーパー マリオ USA、スーパー マリオ ワールドを組み合わせたもので、新しいゲームと追加コンテンツも含まれています<page>これはオープン ソース プロジェクトであることに注意してください。 そのため、バグが発生する可能性があることをご理解ください。 それらを GitHub ページで報告することを躊躇しないでください<page>https://github.com/SpencerEverly/smasplusplus/ で見つけることができます<page>それ以外はゲームをお楽しみください!<page>ただし、始める前に、このゲームではいくつかの前提条件のオプションを設定する必要があります。<question FirstBootMenuOne>",
    --SMAS++ MAIN MENU: First Boot: Date and Time
    ["0x0000000000000002"] = "<setPos 400 32 0.5 -1.2><boxStyle smw>以下の日付と時刻を確認してください (右下隅にあるはずです)。 その時刻とシステムの日付は正しいですか?<question FirstBootMenuTwo>",
    --SMAS++ MAIN MENU: First Boot: Your Name, Please
    ["0x0000000000000003"] = "<setPos 400 32 0.5 -1.4><boxStyle smw>そして今、私たちはあなたの名前を知る必要があります. 名前は何？ [開始] を押して、名前を入力します (これにはキーボードを使用する必要があります)。<question FirstBootMenuThree>",
    --SMAS++ MAIN MENU: First Boot: Inputs for the Game
    ["0x0000000000000004"] = "<setPos 400 32 0.5 -1.2><boxStyle smw>最後に、ゲームの入力を構成する必要があります。 コントロールの設定を開始するには、開始を選択してください。 1.3 モードでプレイしている場合は、後でプレーヤー 2 のコントロールを構成する必要があります。<question FirstBootMenuKeyboard>",
    --SMAS++ MAIN MENU: First Boot: Do you know how to play?
    ["0x0000000000000005"] = "<setPos 400 32 0.5 -1.4><boxStyle smw>あなたは上手い！ 始める前に、ゲームの遊び方を知っていますか?<page>すでに遊び方を知っている場合は、スキップを選択できます。<question FirstBootMenuFour>",
    --SMAS++ MAIN MENU: First Boot: Without further ado...
    ["0x0000000000000006"] = "<setPos 400 32 0.5 -3.2><boxStyle smw>そして今、スーパーマリオコレクション++！<question FirstBootMenuFive>",
    --SMAS++ MAIN MENU: First Boot: Game Help Level
    ["0x0000000000000007"] = "<setPos 400 32 0.5 -0.3><boxStyle smw>ゲームのプレイに関するヘルプを表示するには、ゲーム ヘルプ レベルにリダイレクトする必要があります。 レベルに入りますか、それともスキップしますか? 必要に応じて、後でタイトル画面でゲーム ヘルプを再度リロードできます。<question FirstBootMenuGameHelp>",
    --SMAS++ MAIN MENU: First Boot: Time Fix
    ["0x0000000000000008"] = "<setPos 400 32 0.5 -0.6><boxStyle smw>問題に応じて、https://support.kaspersky.com/common/windows/3508<page> などのサイトにアクセスするか、Google/DuckDuckGo を使用して「fixing system time」を検索してください。<page>最初に時刻を修正してください。修正後、時刻は自動的に更新されます。 「再確認」を押すと、再度時刻を確認できます。<question FirstBootMenuTimeFix>",
    ...
    --ME AND LARRY CITY: Welcome Sign
    ["Welcome to Me and Larry City!<page>This is the central HUB for free powerups, information, and additional stars.<page>To leave the city, head left.<page>To check out the HUB's side quest levels, or the DLC content, check the TV by the sign.<page>To head into the city, keep walking right.<page>To quickly warp between city sections, use the Barrel Cannon or the Pipe above."] = "ミーリアンとラリアンの街へようこそ!<page>これは、無料のパワーアップ、情報、および追加のスターの中心的なハブです<page>街を出るには、左折して歩き続けます<page>ハブのサイド クエスト レベルや DLC コンテンツを確認するには、テレビの看板を確認してください<page>街に向かうには、右に歩き続けます<page>都市のセクション間をすばやくワープするには、バレル キャノンまたは上部のパイプを使用します。",
    ["Greetings, new hero! Welcome to our fine city!"] = "こんにちは、新しいヒーロー! 私たちの素敵な街へようこそ!",
}