...
local zoomedBuffer = Graphics.CaptureBuffer()
local screenBuffer = Graphics.CaptureBuffer()
local backgroundBuffer = Graphics.CaptureBuffer()
...
customCamera.ignoreControllers = false
customCamera.ignoreBlockers = false
...
local function getCurrentSettings()
    ...
    if customCamera.controllerID > 0 and not customCamera.ignoreControllers and EventManager.onStartRan then
        -- Collect active controllers, then sort
        local controllers = {}
        
        local chiefPlayer = general.getChiefPlayer()
        local x = chiefPlayer.x + chiefPlayer.width*0.5
        local y = chiefPlayer.y + chiefPlayer.height

        for _,b in Block.iterateIntersecting(x - 0.01,y - 0.01,x + 0.01,y + 0.01) do
...
local function getExtraBounds()
    ...
    if customCamera.blockerID == 0 or customCamera.ignoreBlockers then
        return bounds
    end
...
function customCamera.onTick()
    ...
    -- Interpolate options between old and new
    local transitionTime = customCamera.settingsTransition
    local easedTransitionTime = easing.inOutQuad(transitionTime,0,1,1)
    
    customCamera.currentZoom = math.lerp(newSettings.zoom,oldSettings.zoom,easedTransitionTime)
    customCamera.currentRotation = math.lerp(newSettings.rotation,oldSettings.rotation,easedTransitionTime)
    customCamera.currentOffsetX = math.lerp(newSettings.offsetX,oldSettings.offsetX,easedTransitionTime)
    customCamera.currentOffsetY = math.lerp(newSettings.offsetY,oldSettings.offsetY,easedTransitionTime)
    customCamera.targetNPCID = newSettings.targetNPCID

    customCamera.screenWidth = math.floor(math.lerp(newSettings.screenWidth,oldSettings.screenWidth,easedTransitionTime) + 0.5)
    customCamera.screenHeight = math.floor(math.lerp(newSettings.screenHeight,oldSettings.screenHeight,easedTransitionTime) + 0.5)
    customCamera.screenOffsetX = math.lerp(newSettings.screenOffsetX,oldSettings.screenOffsetX,easedTransitionTime)
    customCamera.screenOffsetY = math.lerp(newSettings.screenOffsetY,oldSettings.screenOffsetY,easedTransitionTime)
...
function customCamera.onWarp(warp,pl)
    customCamera.resetCameraState()
end

function customCamera.onFramebufferResize(width,height)
    zoomedBuffer = Graphics.CaptureBuffer()
    screenBuffer = Graphics.CaptureBuffer()
    backgroundBuffer = Graphics.CaptureBuffer()
end


function customCamera.onInitAPI()
    registerEvent(customCamera,"onTick")
    registerEvent(customCamera,"onCameraUpdate","onCameraUpdate",false)
    registerEvent(customCamera,"onCameraDraw")

    registerEvent(customCamera,"onStart")

    registerEvent(customCamera,"onWarp")
    registerEvent(customCamera,"onFramebufferResize")

    registerCustomEvent(customCamera,"onPreResetState")
    registerCustomEvent(customCamera,"onPostResetState")
