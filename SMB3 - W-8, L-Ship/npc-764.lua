...
local configFile = npcManager.setNpcSettings(burnerRightSettings)

npcManager.registerDefines(npcID, {NPC.UNHITTABLE})

function burnerRight.onInitAPI()
	npcManager.registerEvent(npcID, burnerRight, "onStartNPC")
	npcManager.registerEvent(npcID, burnerRight, "onTickNPC")
	registerEvent(burnerRight, "onTick")
end
...
function burnerRight.onStartNPC(v)
	local data = v.data
...
function burnerRight.onTickNPC(v)
	...
	if data._settings.isGlobal then
		...
		if globalState == 1 then
			if globalTimer == 1 then
				SFX.play(42)
				data.flame = NPC.spawn(configFile.fireID, v.x + v.width, v.y + 2, v:mem(0x146, FIELD_WORD))
